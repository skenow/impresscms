...
function make_data(&$dbm, &$cm, $adminname, $adminpass, $adminmail, $language, $adminsalt, $gruops){
   ...
   $dbm->insert("image", " (image_id, image_name, image_nicename, image_mimetype, image_created, image_display, image_weight, imgcat_id) VALUES (1, 'img482278e29e81c.png', 'ImpressCMS', 'image/png', ".time().", 1, 0, 1)");
    ...
    $dbm->insert("modules", " VALUES (1, '"._MI_SYSTEM_NAME."', 110, ".$time.", 0, 1, 'system', 0, 1, 1, 0, 0, 0, " . ICMS_SYSTEM_DBVERSION . ")");
	...
	// Insert data for ConfigSec Options in selection field. (must be placed before $is++)
    	$dbm->insert('configoption', " VALUES ($ci, '_MD_AM_NONE', '0', $i)");
	$ci++;
    	$dbm->insert('configoption', " VALUES ($ci, '_MD_AM_SESS_BROWSER', '1', $i)");
	$ci++;
    	$dbm->insert('configoption', " VALUES ($ci, '_MD_AM_SESS_IPAB', '2', $i)");
	$ci++;
    	$dbm->insert('configoption', " VALUES ($ci, '_MD_AM_SESS_IPABC', '3', $i)");
	$ci++;
    	$dbm->insert('configoption', " VALUES ($ci, '_MD_AM_SESS_IPABCD', '4', $i)");
	$ci++;
	// ----------
	$i++;
	$p++;
	$dbm->insert('config', " VALUES ($i, 0, $c, 'session_ip_group', '_MD_AM_SESSIP_GROUP', '".addslashes(serialize(array('1')))."', '_MD_AM_SESSIP_GROUPDSC', 'group_multi', 'array', $p)");
	$i++;
	$p++;
