...
class XoopsConfigItemHandler extends XoopsObjectHandler
{
    ...
    function insert(&$config)
    {
        ...
        if ($config->isNew()) {
            ...
            $sql = sprintf("INSERT INTO %s (conf_modid, conf_catid, conf_name, conf_title, conf_value, conf_desc, conf_formtype, conf_valuetype, conf_order) VALUES ('%u', '%u', %s, %s, %s, %s, %s, %s, '%u')", $this->db->prefix('config'), intval($conf_modid), intval($conf_catid), $this->db->quoteString($conf_name), $this->db->quoteString($conf_title), $this->db->quoteString($conf_value), $this->db->quoteString($conf_desc), $this->db->quoteString($conf_formtype), $this->db->quoteString($conf_valuetype), intval($conf_order));
        } else {
