...
switch($action)
{
	case 'results':
		...
		if($andor != 'exact')
		{
			foreach($queries as $q) {echo ' <b>'.htmlspecialchars(stripslashes($q)).'</b>';}
			if(!empty($ignored_queries))
			{
				echo '<br />';
				printf(_SR_IGNOREDWORDS, $xoopsConfigSearch['keyword_min']);
				foreach($ignored_queries as $q) {echo ' <b>'.htmlspecialchars(stripslashes($q)).'</b>';}
			}
		}
		else {echo ' "<b>'.htmlspecialchars(stripslashes($queries[0])).'</b>"';}
		echo '<br />';
		foreach($mids as $mid)
		{
			$mid = intval($mid);
	...
	case 'showall':
		case 'showallbyuser':
			...
			if(is_array($results) && $count > 0)
			{
				...
				$has_next = false;
				if(is_array($next_results) && $next_count == 1) {$has_next = true;}
				$xoopsTpl->assign('xoops_pagetitle', _SR_SEARCHRESULTS);
				...
				if($action=='showallbyuser') {$search_url .= '&uid=$uid';}
				if($start > 0)
				{
					$prev = $start - 20;
					echo '<td align="left">';
