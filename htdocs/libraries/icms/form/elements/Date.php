...
class icms_form_elements_Date extends icms_form_elements_Text {
	...
	public function render() {
		...
		$result = "<div class='input-append datepicker-wrapper' data-module='app/widgets/datepicker/main'>";
			$result .= "<input type='text' name='".$ele_name."' id='".$ele_name."' maxlength='".$this->getMaxlength()."' value='".date(_SHORTDATESTRING, $ele_value)."'".$this->getExtra()." />";
			$result .= "<span class='add-on'><i class='icon icon-calendar'></i></span>";
		$result .= "</div>";
		// $result = "<input type='text' name='".$ele_name."' id='".$ele_name."' size='".$this->getSize()."' maxlength='".$this->getMaxlength()."' value='".date(_SHORTDATESTRING, $ele_value)."'".$this->getExtra()." />&nbsp;&nbsp;<img src='" . ICMS_URL . "/images/calendar.png' alt='"._CALENDAR."' title='"._CALENDAR."'>";
