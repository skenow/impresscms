 ...
 */
abstract class icms_properties_Handler
            ...
            switch ($this->_vars[$name][self::VARCFG_TYPE]) {
                ...
                case self::DTYPE_CRITERIA;
                    if (!$this->_vars[$name])
                        ...
                        return $this->_values[$name]->render();
        ...
        protected function initVar($key, $dataType, $defaultValue = null, $required = false, $otherCfg = null) {
            ...
            switch ($dataType) {
                ...
                case self::DTYPE_DEP_MTIME:
                    $this->_vars[$key][self::VARCFG_FORMAT] = _('Y/n/j G:i');
                    $this->_vars[$key][self::VARCFG_DEP_DATA_TYPE] = $dataType;
                    $dataType = self::DTYPE_DATETIME;
                break;
                case self::DTYPE_DEP_STIME:
                    $this->_vars[$key][self::VARCFG_FORMAT] = _('Y/n/j');
                    $this->_vars[$key][self::VARCFG_DEP_DATA_TYPE] = $dataType;
                    $dataType = self::DTYPE_DATETIME;
                break;
        ...
        protected function cleanVar($key, $type, $value) {
            switch ($type) {
                ...
                case self::DTYPE_DATA_SOURCE:
                    ...
                    return ($value instanceOf icms_core_ObjectHandler)?$value:null;
                break;
                case self::DTYPE_BOOLEAN:     
                    if (is_bool($value))
                        return $value;
...
//                            unset($data[self::VARCFG_VALUE]);
                        ...
                        $ret[$name] = base64_encode(gzdeflate(json_encode(array(get_class($this->_values[$name]), $vars)), 9));
                    } else {
                        $ret[$name] = null;
                    }
                break;
                case self::DTYPE_CRITERIA:
                    if ($this->_values[$name] instanceof icms_db_criteria_Item)
                        $ret[$name] = $this->_values[$name]->render();
                    else
                        $ret[$name] = '';
                break;
                case self::DTYPE_LIST:
                    $ret[$name] = implode($this->_vars[$name][self::VARCFG_SEPARATOR], $this->_values[$name]);
                break;
                default:                    
