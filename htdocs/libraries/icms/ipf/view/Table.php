...
class icms_ipf_view_Table {
	...
	public function setSortOrder() {
		...
		if ($this->_tempObject->getVarInfo($this->_sortsel, 'itemName', null)) {
	...
	public function render($fetchOnly = false, $debug = false) {
		...
		foreach ($this->_columns as $column) {
			...
			if ($column->_keyname == 'checked') {
				...
				$aColumn['caption'] = isset($this->_tempObject->getVarInfo($column->getKeyName(), 'form_caption')) ? $this->_tempObject->vars[$column->getKeyName()]['form_caption'] : $column->getKeyName();
			}
