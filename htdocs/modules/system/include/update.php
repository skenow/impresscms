...
function xoops_module_update_system(& $module) {
	...
	if ($dbVersion <= $newDbVersion) {
		...
		$result = $icmsDatabaseUpdater->db->query("SELECT imgcat_id FROM " . $icmsDatabaseUpdater->db->prefix('imagecategory') . " WHERE imgcat_name = 'Logos'");
		list ($categ_id) = $icmsDatabaseUpdater->db->fetchRow($result);
	...
	if ($dbVersion < $newDbVersion) {
		echo "Database migrate to version " . $newDbVersion . "<br />";
		$icmsDatabaseUpdater->db->queryF("DELETE FROM `" . $icmsDatabaseUpdater->db->prefix('config') . "` WHERE conf_name='use_jsjalali'");
		$icmsDatabaseUpdater->insertConfig(XOOPS_CONF_PERSONA, 'use_jsjalali', '_MD_AM_JALALICAL', '0', '_MD_AM_JALALICALDSC', 'yesno', 'int', 23);
		unset ($table);
	}
	...
	if ($dbVersion < $newDbVersion) {
		...
		$icmsDatabaseUpdater->insertConfig(XOOPS_CONF, 'admin_session_name', '_MD_AM_ADMINSESSNAME', 'icms_adminsession', '_MD_AM_ADMINSESSNAMEDSC', 'textbox', 'text', 22);
		$icmsDatabaseUpdater->insertConfig(XOOPS_CONF, 'admin_session_expire', '_MD_AM_ADMINSESSEXPIRE', 15, '_MD_AM_ADMINSESSEXPIREDSC', 'textbox', 'int', 23);
	}
