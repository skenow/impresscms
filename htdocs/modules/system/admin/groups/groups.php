...
function modifyGroup($g_id)
{
	...
	$usercount = $member_handler->getUserCount(new icms_core_Criteria('level', 0, '>'));
	$member_handler =& xoops_gethandler('member');
	...
	if ($usercount < 200 && $membercount < 200) {
		...
		if (count($members) > 0) {
			$member_criteria = new icms_core_Criteria('uid', "(".implode(',', $members).")", "IN");
			$member_criteria->setSort('uname');
			$mlist = $member_handler->getUserList($member_criteria);
		}
		$criteria = new icms_core_Criteria('level', 0, '>');
