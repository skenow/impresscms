...
if (! is_object ( $icmsUser ) || ! is_object ( $icmsModule ) || ! $icmsUser->isAdmin ( $icmsModule->mid () )) {
	...
	if ($op == 'show') {
		...
		$criteria->add ( new core_Criteria ( 'conf_modid', 0 ) );
		$criteria->add ( new core_Criteria ( 'conf_catid', $confcat_id ) );
		...
		for($i = 0; $i < $confcount; $i ++) {
			...
			switch ( $config [$i]->getVar ( 'conf_formtype' )) {
				...
				case 'select' :
					...
					$options = $config_handler->getConfigOptions ( new core_Criteria ( 'conf_id', $config [$i]->getVar ( 'conf_id' ) ) );
				...
				case 'startpage' :
					...
					$criteria->add ( new core_Criteria ( 'page_url', '%*', 'NOT LIKE' ) );
				...
				case 'module_cache' :
					$module_handler = & xoops_gethandler ( 'module' );
					$modules = $module_handler->getObjects ( new core_Criteria ( 'hasmain', 1 ), true );
	...
	if ($op == 'showmod') {
		...
		$config = $config_handler->getConfigs ( new core_Criteria ( 'conf_modid', $mod ) );
		$count = count ( $config );
		if ($count < 1) {
			redirect_header ( 'admin.php?fct=preferences', 1 );
		}
                ...
                if(is_array($new_value) || $new_value != $config->getVar('conf_value'))
                {
					...
					if (! $tpl_updated && $config->getVar ( 'conf_catid' ) == ICMS_CONF && $config->getVar ( 'conf_name' ) == 'template_set') {
						...
						if ($xoopsConfig ['template_set'] != ${$config->getVar ( 'conf_name' )}) {
							...
							$imagefiles = & $image_handler->getObjects ( new core_Criteria ( 'tplset_name', $newtplset ), true );
