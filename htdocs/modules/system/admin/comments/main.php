...
if ( !is_object($icmsUser) || !is_object($icmsModule) || !$icmsUser->isAdmin($icmsModule->getVar('mid')) ) {
	...
	switch ($op) {
		case 'list':
			...
			$module_array =& $module_handler->getList(new icms_core_Criteria('hascomments', 1));
			$comment_handler =& xoops_gethandler('comment');
			$criteria = new icms_core_CriteriaCompo();
			if ($status > 0) {
				$criteria->add(new icms_core_Criteria('com_status', $status));
			}
			if ($module > 0) {
				$criteria->add(new icms_core_Criteria('com_modid', $module));
			}
