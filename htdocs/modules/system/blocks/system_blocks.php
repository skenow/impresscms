...
function b_system_online_show()
{
	...
	if (false != $onlines) {
		...
		if (is_object($icmsModule)) {
			$mytotal = $online_handler->getCount(new core_Criteria('online_module', $icmsModule->getVar('mid')));
...
function b_system_main_show()
{
	...
	$criteria->add(new core_Criteria('weight', 0, '>'));
...
function b_system_user_show()
{
	...
	if (is_object($icmsUser)) {
		...
		$criteria->add(new core_Criteria('to_userid', $icmsUser->getVar('uid')));
...
function b_system_newmembers_show($options)
{
	...
	$block = array();
	$criteria = new core_CriteriaCompo(new core_Criteria('level', 0, '>'));
	...
	if ( !empty($options[2]) && $options[2] == 1 ) {
		...
		$block['registered'] = icms_conv_nr2local($member_handler->getUserCount(new core_Criteria('level')));
		$block['inactive'] = icms_conv_nr2local($member_handler->getUserCount(new core_Criteria('level', 0)));
		$block['active'] = icms_conv_nr2local($member_handler->getUserCount(new core_Criteria('level', 0, '>')));
...
function b_system_topposters_show($options)
{
	...
	$size = count($options);
	for ( $i = 2; $i < $size; $i++) {
		$criteria->add(new core_Criteria('rank', $options[$i], '<>'));
	}
	$criteria->setOrder('DESC');
...
function b_system_comments_show($options)
{
	...
	$criteria = new core_CriteriaCompo(new core_Criteria('com_status', XOOPS_COMMENT_ACTIVE));
	...
	$criteria1 = new core_CriteriaCompo(new core_Criteria('gperm_name','module_read','='));
	$criteria1->add(new core_Criteria('gperm_groupid', '('.implode(',', $gperm_groupid).')', 'IN'));
	...
	if(count($modIds) > 0 ) {
		...
		$criteria->add(new core_Criteria('com_modid', '('.implode(',', $modIds).')', 'IN'));
	}
	// Check modules permissions
	...
	$member_handler =& xoops_gethandler('member');
	$module_handler =& xoops_gethandler('module');
	$modules = $module_handler->getObjects(new core_Criteria('hascomments', 1), true);
