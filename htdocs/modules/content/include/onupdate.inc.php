...
function icms_module_update_content($module) {
	...
	if ($table->exists ()) {
		...
		while ( $row = $db->fetchArray ( $result ) ) {
			...
			$crit = new core_CriteriaCompo ( new core_Criteria ( 'gperm_name', 'content_read' ) );
			$crit->add ( new core_Criteria ( 'gperm_name', 'content_admin' ), 'OR' );
			$criteria->add ( $crit );
			$criteria->add ( new core_Criteria ( 'gperm_itemid', $row ['content_id'] ) );
			...
			$criteria = new core_CriteriaCompo ( new core_Criteria ( 'page_url', '%' . $old_seo, 'LIKE' ) );
			$criteria->add ( new core_Criteria ( 'page_moduleid', 1 ) );
			$symlinks_remove = $symlink_handler->getObjects ( $criteria );
				
			$criteria = new core_CriteriaCompo ( new core_Criteria ( 'page_url', '%' . $url, 'LIKE' ) );
		...
		//Content Block
		$criteria = new core_CriteriaCompo ( new core_Criteria ( 'show_func', 'content_content_display_show' ) );
		...
		$criteria = new core_CriteriaCompo ( new core_Criteria ( 'show_func', 'b_content_show' ) );
		...
		foreach ( $content_blocks as $block ) {
			...
			if ($icms_block_handler->insert ( $nb, true )) {
				$criteria = new core_CriteriaCompo ( new core_Criteria ( 'gperm_name', 'block_read' ) );
				$criteria->add ( new core_Criteria ( 'gperm_itemid', $block->getVar ( 'bid', 'e' ) ) );
				$criteria->add ( new core_Criteria ( 'gperm_modid', 1 ) );
		...
		//Content Menu Block
		$criteria = new core_CriteriaCompo ( new core_Criteria ( 'show_func', 'content_content_menu_show' ) );
		...
		$criteria = new core_CriteriaCompo ( new core_Criteria ( 'show_func', 'b_content_menu_show' ) );
		$criteria->add ( new core_Criteria ( 'show_func', 'b_content_relmenu_show' ), 'OR' );
