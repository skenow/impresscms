m = {
    controlInitialized: function () {
        ...
        alink.css({
           /* 'background-image': window.ImpressCMS.config.url.root+'/images/add.gif',*/
        ...
        window.ImpressCMS.core.controls.waitUntilControlExists(id, function () {
        ...
        switch (this.getLinkedControlType()) {
            case 'icms/table':
                ...
                if (!dt[1])
                    return false;
                for(var x in dt[1]) {
                    this.cachedData.fields.types.push(dt[1][x][window.ImpressCMS.config.var_param.type]);
                    this.cachedData.fields.names.push(x);
                    this.cachedData.fields.captions.push(dt[1][x][window.ImpressCMS.config.var_param.form_caption]);
                    var cmp = this.getComparisionOptionsByFieldType(dt[1][x][window.ImpressCMS.config.var_param.type]);                    
                    if (dt[1][x][window.ImpressCMS.config.var_param.data_handler] == undefined) {
    ...
    getLinkedControlInstance: function () {
        var control = window.ImpressCMS.core.controls.getControl(this.getVar('linked_control'));        
        return control;
    },
    ...
    addFilterLine: function (field, comparision, value) {
        ...
        var rimg = jQuery('<img src="'+window.ImpressCMS.config.url.root+'/images/icons/delete.gif" title="Remove" alt="Remove" />');
        rlink.append(rimg);
        rlink.click(
            function () {                
                div.remove();
