 ...
 * @version		$Id: Base.php 11512 2011-12-28 04:19:27Z skenow $
 */
		...
		foreach ($vars as $key=>$var) {
			...
			if ($key == $this->targetObject->handler->keyName || !$var['displayOnForm']) {
				...
				if (isset($controls[$key])) {
					...
					switch (isset($var[icms_ipf_Properties::VARCFG_DEP_DATA_TYPE])?$var[icms_ipf_Properties::VARCFG_DEP_DATA_TYPE]:$var['data_type']) {
                        ...
                        case icms_ipf_Properties::DTYPE_BOOLEAN:
                            $form_obj = $this->getControl('yesno', $key);
							$this->addElement($form_obj, $key, $var);
							unset($form_obj);
							break;
