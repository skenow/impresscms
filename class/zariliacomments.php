<?php
// $Id: zariliacomments.php,v 1.2 2007/04/22 07:21:32 catzwolf Exp $
// ------------------------------------------------------------------------ //
...
include_once ZAR_ROOT_PATH . '/language/' . $GLOBALS['zariliaConfig']['language'] . '/comment.php';

class ZariliaComments extends ZariliaObject {
    var $ctable;
    var $db;

    function ZariliaComments( $ctable, $id = null ) {
        $this->ctable = $ctable;
    ...
    function delete() {
        ...
        $mytree = new ZariliaTree( $this->ctable, "comment_id", "pid" );
    ...
    function &getCommentTree() {
        ...
        foreach ( $tarray as $ele ) {
            $ret[] = new ZariliaComments( $this->ctable, $ele );
        }
        return $ret;
    }
    ...
    function printNavBar( $item_id, $mode = "flat", $order = 1 ) {
        ...
        echo "<form method='get' op='" . $_SERVER['PHP_SELF'] . "'><table width='100%' border='0' cellspacing='1' cellpadding='2'><tr><td class='bg1' align='center'><select name='mode'><option value='nocomments'";
        if ( $mode == "nocomments" ) {
        ...
        if ( $zariliaConfig['anonpost'] == 1 || $zariliaUser ) {
            if ( $mode != "flat" || $mode != "nocomments" || $mode != "thread" ) {
                $mode = "flat";
            }
            echo "&nbsp;<input type=\"button\" class=\"formbutton\" onclick=\"window.location='newcomment.php?item_id=" . intval( $item_id ) . "&amp;order=" . intval( $order ) . "&amp;mode=" . $mode . "'\" value=\"" . _CM_POSTCOMMENT . "\" />";
        }
    ...
    function showThreadPost( $order, $mode, $adminview = 0, $color_num = 1 ) {
        ...
        if ( $this->getVar( "icon" ) != null && $this->getVar( "icon" ) != "" ) {
            $subject_image = "<a name='" . $this->getVar( "comment_id" ) . "' id='" . $this->getVar( "comment_id" ) . "'></a><img src='" . ZAR_URL . "/images/subject/" . $this->getVar( "icon" ) . "' alt='' />";
        } else {
            $subject_image = "<a name='" . $this->getVar( "comment_id" ) . "' id='" . $this->getVar( "comment_id" ) . "'></a><img src='" . ZAR_URL . "/images/icons/no_posticon.gif' alt='' />";
        }
        if ( $adminview ) {
            $ip_image = "<img src='" . ZAR_URL . "/images/icons/ip.gif' alt='" . $this->getVar( "ip" ) . "' />";
        } else {
            $ip_image = "<img src='" . ZAR_URL . "/images/icons/ip.gif' alt='' />";
        }
        if ( $adminview || ( $zariliaUser && $this->getVar( "user_id" ) == $zariliaUser->getVar( "uid" ) ) ) {
            $edit_image = "<a href='editcomment.php?comment_id=" . $this->getVar( "comment_id" ) . "&amp;mode=" . $mode . "&amp;order=" . intval( $order ) . "'><img src='" . ZAR_URL . "/images/icons/edit.gif' alt='" . _EDIT . "' /></a>";
        }
        if ( $zariliaConfig['anonpost'] || $zariliaUser ) {
            $reply_image = "<a href='replycomment.php?comment_id=" . $this->getVar( "comment_id" ) . "&amp;mode=" . $mode . "&amp;order=" . intval( $order ) . "'><img src='" . ZAR_URL . "/images/icons/reply.gif' alt='" . _REPLY . "' /></a>";
        }
        if ( $adminview ) {
            $delete_image = "<a href='deletecomment.php?comment_id=" . $this->getVar( "comment_id" ) . "&amp;mode=" . $mode . "&amp;order=" . intval( $order ) . "'><img src='" . ZAR_URL . "/images/icons/delete.gif' alt='" . _DELETE . "' /></a>";
        }
        ...
        if ( $poster ) {
            ...
            $rank = $poster->rank();
            if ( $rank['image'] != "" ) {
                $rank['image'] = "<img src='" . ZAR_UPLOAD_URL . "/" . $rank['image'] . "' alt='' />";
            }
            $avatar_image = "<img src='" . ZAR_UPLOAD_URL . "/" . $poster->getVar( "user_avatar" ) . "' alt='' />";
            if ( $poster->isOnline() ) {
            ...
            if ( $poster->getVar( "user_viewemail" ) ) {
                $email_image = "<a href='mailto:" . $poster->getVar( "email", "E" ) . "'>Email</a>";
            } else {
                $email_image = "";
            }
            ...
            showThread( $color_num, $subject_image, $this->getVar( "subject" ), $this->getVar( "comment" ), $post_date, $ip_image, $reply_image, $edit_image, $delete_image, $zariliaConfig['anonymous'] );
        }
    }
