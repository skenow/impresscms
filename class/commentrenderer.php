<?php
// $Id: commentrenderer.php,v 1.1 2007/03/16 02:38:58 catzwolf Exp $
// ------------------------------------------------------------------------ //
...
class ZariliaCommentRenderer {
	 ...
	 **/
	function ZariliaCommentRenderer(&$tpl, $use_icons = true, $do_iconcheck = false)
	{
		$this->_tpl =& $tpl;
	...
	function &instance(&$tpl, $use_icons = true, $do_iconcheck = false)
	{
		static $instance;
		if (!isset($instance)) {
			$instance = new ZariliaCommentRenderer($tpl, $use_icons, $do_iconcheck);
		}
		return $instance;
	}
	 ...
	 * Accessor
	 * 
	 * @param   object  &$comments_arr  array of {@link ZariliaComment} objects
	 **/
	...
	function renderFlatView($admin_view = false)
	{
		...
		for ($i = 0; $i < $count; $i++) {
			...
			if (false != $admin_view) {
				...
				if (ZAR_COMMENT_ACTIVE != $this->_comments[$i]->getVar('com_status')) {
					continue;
				} else {
	...
	function renderThreadView($comment_id = 0, $admin_view = false, $show_nav = true)
	{
		...
		$xot = new ZariliaObjectTree($this->_comments, 'com_id', 'com_pid', 'com_rootid');
		$tree =& $xot->getTree();
		...
		if (false != $admin_view) {
			...
			// hide comments that are not active
			if (ZAR_COMMENT_ACTIVE != $tree[$comment_id]['obj']->getVar('com_status')) {
	...
	function _renderThreadReplies(&$thread, $key, &$replies, $prefix, $admin_view, $depth = 0, $current_prefix = '')
	{
        ...
        if (isset($thread[$key]['child']) && !empty($thread[$key]['child'])) {
            ...
            foreach ($thread[$key]['child'] as $childkey) {
                if (!$admin_view && $thread[$childkey]['obj']->getVar('com_status') != ZAR_COMMENT_ACTIVE) {
	...
	function _getPosterName($poster_id)
	{
		...
		$poster['uname'] = $GLOBALS['zariliaConfig']['anonymous'];
		return $poster;
	}
	...
	function _getTitleIcon($icon_image)
	{
		if ($icon_image != '') {
			if (false != $this->_doIconCheck) {
				if (!file_exists(ZAR_URL.'/images/subject/'.$icon_image)) {
					return '<img src="'.ZAR_URL.'/images/icons/no_posticon.gif" alt="" />';
				} else {
					return '<img src="'.ZAR_URL.'/images/subject/'.$icon_image.'" alt="" />';
				}
			} else {
				return '<img src="'.ZAR_URL.'/images/subject/'.$icon_image.'" alt="" />';
			}
