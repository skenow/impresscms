...
class DHTML_Calendar {
    ...
    function load_files() {
		...
		if (!isset($zariliaOption['CalendarLoaded'])) {
			$zariliaTpl->headerAdd( $this->get_load_files_code() );
			$zariliaOption['CalendarLoaded'] = true;
		}
    ...
    function make_input_field($cal_options = array(), $field_attributes = array(), $show = true) {
        ...
        $data = '<input ' . $attrstr .'/>';
        $data .= '<a href="#" id="'. $this->_trigger_id($id) . '">' .
            '<img align="middle" alt="button" title="title" border="0" src="' . $this->calendar_lib_path . 'img.gif" alt="" /></a>';
