 ...
 * @access public
 */
class ZariliaAnyDatabase extends ZariliaDatabase {
    var $dbcon;
    // var $_path = '';
    /**
     * ZariliaAnyDatabase::connect()
     *
     * @param boolean $selectdb
     * @return
     */
    function connect( $selectdb = true, $show_error = false ) {
		$this->dbcon = ADONewConnection(ZAR_DB_TYPE);
    ...
    /**
     * ZariliaAnyDatabase::getInsertId()
     *
     * @return
     */
    ...
    /**
     * ZariliaAnyDatabase::fetchObject()
     *
     * @param  $result
     * @return
     */
    ...
    /**
     * ZariliaAnyDatabase::getAffectedRows()
     *
     * @return
     */
    ...
    /**
     * Returns the text of the error message from previous Any operation
     *
     * @return bool Returns the error text from the last Any function, or '' (the empty string) if no error occurred.
     */
    function error() {
    ...
    /**
     * Returns the numerical value of the error message from previous Any operation
     *
     * @return int Returns the error number from the last Any function, or 0 (zero) if no error occurred.
     */
    function errno() {
        return @mysql_errno();
    }
     ...
     *
     * @param string $sql a valid Any query
    ...
    function queryF( $sql, $limit = 0, $start = 0 ) {
        return $this->SelectLimit( $sql, $limit, $start );
    }
    ...
    function getServerInfo( $info = false ) {
        if ( $info == true ) {
            return printf( "Any server version: %s\n", mysql_get_server_info() );
        } else {
    ...
    function getProtoInfo( $info = false ) {
        if ( $info == true ) {
            return printf( "Any protocol version: %s\n", mysql_get_proto_info() );
        } else {
    ...
    function getClientInfo( $info = false ) {
        if ( $info == true ) {
            return printf( "Any client info: %s\n", mysql_get_client_info() );
        } else {
