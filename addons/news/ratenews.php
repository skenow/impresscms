...
include_once ZAR_ROOT_PATH.'/class/module.errorhandler.php';
...
if(!empty($_POST['submit'])) {			// The form was submited
	...
	if(!is_object($zariliaUser)){
		...
		$ratinguser = $zariliaUser->getVar('uid');
	}
	...
	if ($rating=='--') {
		redirect_header(ZAR_URL.'/addons/news/ratenews.php?storyid='.$storyid,4,_NW_NORATING);
		exit();
	}

	// Check if News POSTER is voting (UNLESS Anonymous users allowed to post)
	if ($ratinguser != 0) {
		$result=$zariliaDB->Execute('SELECT uid FROM '.$zariliaDB->prefix('stories')." WHERE storyid=$storyid");
		while(list($ratinguserDB)=$result->FetchRow()) {
			if ($ratinguserDB==$ratinguser) {
				redirect_header(ZAR_URL.'/addons/news/index.php',4,_NW_CANTVOTEOWN);
				exit();
			}
		}

		// Check if REG user is trying to vote twice.
		$result=$zariliaDB->Execute('SELECT ratinguser FROM '.$zariliaDB->prefix('stories_votedata')." WHERE storyid=$storyid");
		while(list($ratinguserDB)=$result->FetchRow()) {
			if ($ratinguserDB==$ratinguser) {
				redirect_header(ZAR_URL.'/addons/news/index.php',4,_NW_VOTEONCE);
				exit();
			}
		}
		...
		$result=$zariliaDB->Execute('SELECT COUNT(*) FROM '.$zariliaDB->prefix('stories_votedata')." WHERE storyid=$storyid AND ratinguser=0 AND ratinghostname = '$ip'  AND ratingtimestamp > $yesterday");
		list($anonvotecount) = $result->FetchRow();
		if ($anonvotecount >= 1) {
			redirect_header(ZAR_URL.'/addons/news/index.php',4,_NW_VOTEONCE);
			exit();
		}
	...
	$newid = $zariliaDB->genId($zariliaDB->prefix('stories_votedata').'_ratingid_seq');
	$datetime = time();
	$sql = sprintf("INSERT INTO %s (ratingid, storyid, ratinguser, rating, ratinghostname, ratingtimestamp) VALUES (%u, %u, %u, %u, '%s', %u)", $zariliaDB->prefix("stories_votedata"), $newid, $storyid, $ratinguser, $rating, $ip, $datetime);
	$zariliaDB->Execute($sql) or $eh('0013');

	//All is well.  Calculate Score & Add to Summary (for quick retrieval & sorting) to DB.
	news_updaterating($storyid);
	$ratemessage = _NW_VOTEAPPRE.'<br />'.sprintf(_NW_THANKYOU,$zariliaConfig['sitename']);
	redirect_header(ZAR_URL.'/addons/news/index.php',4,$ratemessage);
	exit();
} else {		// Display the form to vote
	$zariliaOption['template_main'] = 'news_ratenews.html';
    include_once ZAR_ROOT_PATH.'/header.php';
    $result=$zariliaDB->Execute('SELECT title FROM '.$zariliaDB->prefix('stories')." WHERE storyid=$storyid");
    list($title) = $result->FetchRow();
    $title = $myts->htmlSpecialChars($title);
	$zariliaTpl->assign('advertisement', news_getmoduleoption('advertisement'));
    $zariliaTpl->assign('news', array('storyid' => $storyid, 'title' => $title));
    $zariliaTpl->assign('lang_voteonce', _NW_VOTEONCE);
    $zariliaTpl->assign('lang_ratingscale', _NW_RATINGSCALE);
    $zariliaTpl->assign('lang_beobjective', _NW_BEOBJECTIVE);
    $zariliaTpl->assign('lang_donotvote', _NW_DONOTVOTE);
    $zariliaTpl->assign('lang_rateit', _NW_RATEIT);
    $zariliaTpl->assign('lang_cancel', _CANCEL);
    $zariliaTpl->addTitle($title . ' - ' . _NW_RATETHISNEWS . ' - ' .  $myts->htmlSpecialChars($zariliaAddon->getVar('name')));
