...
function b_news_top_show($options) {
	...
	$block['imagesurl']=ZAR_URL.'/addons/news/images/';
	...
	if($options[8]==2) {		// Tabbed view ********************************************************************************************
		...
		if($currenttab==0 && $usespotlight) {	// Spotlight or not ?
			...
			$spotlight['title'] = $tmpstory->title();
       		if ($options[7] != '') {
	        	$spotlight['image'] = sprintf("<a href='%s'>%s</a>", ZAR_URL.'/addons/news/article.php?storyid='.$tmpstory->storyid(),$myts->displayTarea($options[7], $tmpstory->nohtml));
			}
       		$spotlight['text'] = $tmpstory->hometext();

       		// Added 16 february 2007 *****************************************
       		$story_user = null;
			$story_user = new ZariliaUser($tmpstory->uid());
			if(is_object($story_user)) {
	       		$spotlight['avatar'] = ZAR_UPLOAD_URL.'/'.$story_user->getVar('user_avatar');
	       	}
			// ****************************************************************
			...
			if(strlen(zarilia_trim($tmpstory->bodytext()))>0) {
				$spotlight['read_more']=true;
			} else {
				$spotlight['read_more']=false;
			}

       		$spotlight['readmore'] = sprintf("<a href='%s'>%s</a>", ZAR_URL.'/addons/news/article.php?storyid='.$tmpstory->storyid(),_MB_READMORE);
       		$spotlight['title_with_link'] = sprintf("<a href='%s'>%s</a>", ZAR_URL.'/addons/news/article.php?storyid='.$tmpstory->storyid(),$tmpstory->title());
       		if($tmpstory->votes()==1) {
			...
			// Added, topic's image and description
   			$spotlight['topic_image']=ZAR_URL.'/addons/news/images/topics/'.$tmpstory->topic_imgurl();
   			...
   			if(count($stories)>0) {
   				foreach ($stories as $key => $story) {
					...
					if (strlen($title) > $options[2]) {
						$title = zarilia_substr($title,0,$options[2]+3);
					}
           			$news['title'] = $title;
            		...
            		if ($options[3] > 0) {
		        		$news['teaser'] = zarilia_substr(strip_tags($story->hometext()), 0, $options[3]+3);
           			} else {
			        	$news['teaser'] = '';
           			}
					if($infotips>0) {
           			...
           			$news['title_with_link'] = sprintf("<a href='%s'%s>%s</a>", ZAR_URL.'/addons/news/article.php?storyid='.$story->storyid(),$news['infotips'],$title);
           			$spotlight['news'][] = $news;
           		}
			}
			...
			if($tabscount>0) {
       			...
       			// Added, topic's image and description
       			$block['topic_image']=ZAR_URL.'/addons/news/images/topics/'.$topic->topic_imgurl();
       			$block['topic_description']=$topic->topic_description();

    			$smallheader=array();
    			$stats=$topic->getTopicMiniStats($thetopic);
    			$smallheader[]=sprintf("<a href='%s'>%s</a>", ZAR_URL.'/addons/news/index.php?storytopic='.$thetopic,_MB_READMORE);
				...
				if(count($stories)>0) {
    				foreach ($stories as $key => $story) {
		        		...
		        		$title = $story->title();
						if (strlen($title) > $options[2]) {
							$title = zarilia_substr(strip_tags($title),0,$options[2]+3);
						}
            			if ($options[7] != '') {
			                $news['image'] = sprintf("<a href='%s'>%s</a>", ZAR_URL.'/addons/news/article.php?storyid='.$story->storyid(),$myts->displayTarea($options[7], $story->nohtml));
            			}
                		if($options[3]>0) {
		                	$news['text'] = zarilia_substr(strip_tags($story->hometext()), 0, $options[3]+3);
                		} else {
							$news['text'] = '';
                		}
            			...
            			$news['title']=sprintf("<a href='%s' %s>%s</a>", ZAR_URL.'/addons/news/article.php?storyid='.$story->storyid(),$news['infotips'],$title);
            			$news['id'] = $story->storyid();
            			$news['date'] = formatTimestamp($story->published(), $dateformat);
            			$news['hits'] = $story->counter();
            			$news['rating'] = number_format($story->rating(), 2);
		...
		if(zarilia_trim($options[9])!='') {
			$block['color1']=$options[9];
		}
		if(zarilia_trim($options[10])!='') {
			$block['color2']=$options[10];
		}
		if(zarilia_trim($options[11])!='') {
			$block['color3']=$options[11];
		}
		if(zarilia_trim($options[12])!='') {
			$block['color4']=$options[12];
		}
		if(zarilia_trim($options[13])!='') {
	    ...
	    foreach ($stories as $key => $story) {
        	...
        	if (($options[5]==0) && ($options[4] == 1) && (($options[6] > 0 && $options[6] == $story->storyid()) || ($options[6] == 0 && $key == 0))) {
        		...
        		if($visible) {
            		...
            		if ($options[7] != '') {
		                $spotlight['image']= sprintf("<a href='%s'>%s</a>", ZAR_URL.'/addons/news/article.php?storyid='.$story->storyid(),$myts->displayTarea($options[7], $story->nohtml));
            		}
		       		// Added 16 february 2007 *****************************************
       				$story_user = null;
					$story_user = new ZariliaUser($story->uid());
					if(is_object($story_user)) {
	       				$spotlight['avatar'] = ZAR_UPLOAD_URL.'/'.$story_user->getVar('user_avatar');
	       			}
					// ****************************************************************
            		...
            		// Added, topic's image and description
		   			$spotlight['topic_image']=ZAR_URL.'/addons/news/images/topics/'.$story->topic_imgurl();
   					$spotlight['topic_description']=$myts->displayTarea($story->topic_description,1);
   					if(strlen(zarilia_trim($story->bodytext()))>0) {
   						$spotlight['read_more']=true;
   					} else {
   						$spotlight['read_more']=false;
   					}
	    ...
	    if (!isset($spotlight) && $options[4]) {
            ...
            if($block['use_spotlight']==true) {
        		$spotlight = array();
        		$spotlight['title'] = zarilia_substr($spotlightArticle->title(),0,($options[2]-1));;
        		if ($options[7] != '') {
		            $spotlight['image'] = sprintf("<a href='%s'>%s</a>", ZAR_URL.'/addons/news/article.php?storyid='.$spotlightArticle->storyid(),$myts->displayTarea($options[7], $spotlightArticle->nohtml));
        		}
	       		// Added 16 february 2007 *****************************************
   				$story_user = null;
				$story_user = new ZariliaUser($spotlightArticle->uid());
				if(is_object($story_user)) {
       				$spotlight['avatar'] = ZAR_UPLOAD_URL.'/'.$story_user->getVar('user_avatar');
       			}
				// ****************************************************************
				...
				// Added, topic's image and description
	   			$spotlight['topic_image']=ZAR_URL.'/addons/news/images/topics/'.$spotlightArticle->topic_imgurl();
        		...
        		if(strlen(zarilia_trim($spotlightArticle->bodytext()))>0) {
					$spotlight['read_more']=true;
				} else {
					$spotlight['read_more']=false;
				}
...
function b_news_top_edit($options) {
    ...
    $form .= "&nbsp;<img align='middle' onmouseover='style.cursor=\"hand\"' onclick='javascript:openWithSelfMain(\"".ZAR_URL."/imagemanager.php?target=spotlightimage\",\"imgmanager\",400,430);' src='".ZAR_URL."/images/image.gif' alt='image' title='image' />";
