...
{
	global $zariliaDB, $zariliaConfig;
	include_once ZAR_ROOT_PATH.'/addons/news/class/class.newsstory.php';
	...
	$sql = "SELECT distinct(FROM_UNIXTIME(published,'%Y-%m')) as published FROM ".$zariliaDB->prefix('stories').' WHERE published>='.$starting_date .' AND published<='.$ending_date.' ORDER BY published '.$sort_order;
