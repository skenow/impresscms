...
include_once ZAR_ROOT_PATH . "/addons/news/class/class.mimetype.php";
...
$this->db=&$zariliaDB;
	...
	function createUploadName($folder,$filename, $trimname=false)
	{
		...
		if(zarilia_substr($workingfolder,strlen($workingfolder)-1,1)<>'/') {
			$workingfolder.='/';
		}
	...
	function delete($workdir=ZAR_UPLOAD_PATH)
    {
    ...
    function updateCounter()
    {
		...
		if(!$result = $this->db->ExecuteF($sql)) {
			return false;
        }
        return true;
    }
