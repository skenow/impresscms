...
class NewsTopic extends ZariliaTopic
{
	var $menu;
...
$this->db=&$zariliaDB;
	...
	function makeMySelBox($title,$order="",$preset_id=0, $none=0, $sel_name="topic_id", $onchange="", $perms)
	{
		...
		while ( list($catid, $name) = $result->FetchRow() ) {
			$sel = "";
			if ( $catid == $preset_id ) {
	...
	*/
	...
	function store()
	{
		...
		if ( $this->use_permission == true ) {
			$xt = new ZariliaTree($this->table, "topic_id", "topic_pid");
			$parent_topics = $xt->getAllParentId($this->topic_id);
			if ( !empty($this->m_groups) && is_array($this->m_groups) ){
				foreach ( $this->m_groups as $m_g ) {
					$moderate_topics = ZariliaPerms::getPermitted($this->mid, "ModInTopic", $m_g);
					$add = true;
			...
			if ( !empty($this->s_groups) && is_array($this->s_groups) ){
				foreach ($this->s_groups as $s_g ) {
					$submit_topics = ZariliaPerms::getPermitted($this->mid, "SubmitInTopic", $s_g);
					$add = true;
					foreach($parent_topics as $p_topic){
			...
			if ( !empty($this->r_groups) && is_array($this->r_groups) ){
				foreach ( $this->s_groups as $r_g ) {
					$read_topics = ZariliaPerms::getPermitted($this->mid, "ReadInTopic", $r_g);
					$add = true;
					foreach($parent_topics as $p_topic){
	...
	function getTopicTitleFromId($topic,&$topicstitles)
	{
	    ...
	    $result = $this->db->Execute($sql);
		while ($row = $this->db->fetchArray($result)) {
			$topicstitles[$row["topic_id"]]=array("title"=>$myts->displayTarea($row["topic_title"]),"picture"=>ZAR_URL.'/addons/news/images/topics/'.$row["topic_imgurl"]);
		}
		return $topicstitles;
	}
