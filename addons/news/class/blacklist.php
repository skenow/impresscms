...
class news_blacklist
{
	...
	function getAllKeywords()
	{
		...
		$filename = ZAR_UPLOAD_PATH.'/news_black_list.php';
		if(file_exists($filename)) {
			include_once $filename;
			foreach($tbl_black_list as $onekeyword) {
				if(zarilia_trim($onekeyword) != '') {
	...
	function addkeywords($keyword)
	{
		...
		if(is_array($keyword)) {
			foreach($keyword as $onekeyword) {
				$onekeyword = zarilia_trim($myts->htmlSpecialChars($onekeyword));
				if($onekeyword != '') {
					$this->keywords[$onekeyword] = $onekeyword;
				}
			}
		} else {
			$keyword = zarilia_trim($myts->htmlSpecialChars($keyword));
			if($keyword != '') {
