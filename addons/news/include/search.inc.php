...
function news_search($queryarray, $andor, $limit, $offset, $userid){
	...
	$sql = "SELECT storyid, topicid, uid, title, created FROM ".$zariliaDB->prefix("stories")." WHERE (published>0 AND published<=".time().") AND (expired = 0 OR expired > ".time().') ';
	...
	$result = $zariliaDB->SelectLimit($sql,$limit,$offset);
	$ret = array();
	$i = 0;
	...
	if($searchincomments && (isset($limit) && $i<=$limit)) {
		...
		$ind=$i;
		$sql = "SELECT com_id, com_modid, com_itemid, com_created, com_uid, com_title, com_text, com_status FROM ".$zariliaDB->prefix("zariliacomments")." WHERE (com_id>0) AND (com_modid=$modid) AND (com_status=".ZAR_COMMENT_ACTIVE.") ";
		if ( $userid != 0 ) {
