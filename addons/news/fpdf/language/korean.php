...
$pdf_config['url'] = ZAR_URL;
$pdf_config['mail'] = 'mailto:'.$zariliaConfig['adminmail'];
$pdf_config['slogan']=zarilia_substr($myts->htmlspecialchars( $zariliaConfig['sitename'] ),0,30);
...
class PDF_language extends PDF_Korean
{
	...
	function _encoding(&$text, $in_charset, $out_charset)
	{
		...
		if(ZAR_USE_MULTIBYTES && function_exists('mb_convert_encoding')) $converted_text = @mb_convert_encoding($text, $out_charset, $in_charset);
		else
