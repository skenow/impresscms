...
$zariliaOption['template_main'] = 'news_archive.html';
...
$zariliaTpl->addTitle( $myts->htmlSpecialChars(_NW_NEWSARCHIVES) . $pgtitle . ' - ' . $myts->htmlSpecialChars($zariliaAddon->getVar('name')));

$useroffset = '';
if(is_object($zariliaUser)) {
	$timezone = $zariliaUser->timezone();
	if(isset($timezone)){
		$useroffset = $zariliaUser->timezone();
	} else {
		$useroffset = $zariliaConfig['default_TZ'];
	}
}
$result = $zariliaDB->Execute('SELECT published FROM '.$zariliaDB->prefix('stories').' WHERE (published>0 AND published<='.time().') AND (expired = 0 OR expired <= '.time().') ORDER BY published DESC');
if (!$result) {
	echo _ERRORS;
	exit();
} else {
	...
	$months = array();
	$i = 0;
	while (list($time) = $result->FetchRow()) {
	...
	$years[$i]['months'] = $months;
	$zariliaTpl->assign('years', $years);
}

if ($fromyear != 0 && $frommonth != 0) {
	$zariliaTpl->assign('show_articles', true);
	$zariliaTpl->assign('lang_articles', _NW_ARTICLES);
	$zariliaTpl->assign('currentmonth', $months_arr[$frommonth]);
	$zariliaTpl->assign('currentyear', $fromyear);
	$zariliaTpl->assign('lang_actions', _NW_ACTIONS);
	$zariliaTpl->assign('lang_date', _NW_DATE);
	$zariliaTpl->assign('lang_views', _NW_VIEWS);

	// must adjust the selected time to server timestamp
	$timeoffset = $useroffset - $zariliaConfig['server_TZ'];
	...
	if(is_array($storyarray) && $count>0) {
		foreach ($storyarray as $article) {
	    	...
	    	$story['title'] = "<a href='".ZAR_URL.'/addons/news/index.php?storytopic='.$article->topicid()."'>".$article->topic_title()."</a>: <a href='".ZAR_URL."/addons/news/article.php?storyid=".$article->storyid()."'".$htmltitle.">".$article->title()."</a>";
	    	$story['counter'] = $article->counter();
	    	$story['date'] = formatTimestamp($article->published(),$dateformat,$useroffset);
	    	$story['print_link'] = ZAR_URL.'/addons/news/print.php?storyid='.$article->storyid();
	    	$story['mail_link'] = 'mailto:?subject='.sprintf(_NW_INTARTICLE, $zariliaConfig['sitename']).'&amp;body='.sprintf(_NW_INTARTFOUND, $zariliaConfig['sitename']).':  '.ZAR_URL.'/addons/'.$zariliaAddon->getVar('dirname').'/article.php?storyid='.$article->storyid();
	    	$zariliaTpl->append('stories', $story);
		}
	}
	$zariliaTpl->assign('lang_printer', _NW_PRINTERFRIENDLY);
	$zariliaTpl->assign('lang_sendstory', _NW_SENDSTORY);
	$zariliaTpl->assign('lang_storytotal', sprintf(_NW_THEREAREINTOTAL, $count));
} else {
    $zariliaTpl->assign('show_articles', false);
}

$zariliaTpl->assign('lang_newsarchives', _NW_NEWSARCHIVES);
