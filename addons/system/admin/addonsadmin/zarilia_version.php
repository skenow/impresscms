<?php
// $Id: zarilia_version.php,v 1.1 2007/03/16 02:36:10 catzwolf Exp $
// ------------------------------------------------------------------------ //
...
$addonversion = array( 'name' => _MA_INFO_NAME,
    ...
    'category' => ZAR_SYSTEM_ADDON
    );
$addonversion['menu']['addons'][] = array( 'url' => ZAR_URL . "/addons/system/index.php?fct=addonsadmin", 'title' => _MD_AM_MDAD_MENU, 'class'=>'administration' );

$addonperm_handler = &zarilia_gethandler( 'groupperm' );
$addon_handler = &zarilia_gethandler( 'addon' );
$admin_mids = $addonperm_handler->getItemIds( 'addon_admin', $zariliaUser->getGroups() );
$criteria = new CriteriaCompo( new Criteria( 'mid', "(" . implode( ',', $admin_mids ) . ")", 'IN' ), true );
$criteria->add( new Criteria( 'hasadmin', 1 ) );
$criteria->add( new Criteria( 'isactive', 1 ) );
$criteria->add( new Criteria( 'mid', 1, '!=' ) );
$criteria->setSort( 'name' );
$criteria->setOrder( 'ASC' );
$installed_mods = &$addon_handler->getObjects( $criteria );
if (count($installed_mods)>0) {
 	$installed_addons[] = array( 'url' => '', 'title' => '<hr />' );
	foreach ( $installed_mods as $mod ) {
		$path = ( $mod->getVar( 'dirname' ) == 'system' ) ? 'index.php?fct=addonsadmin' : 'admin/index.php';
        $addonversion['menu']['addons'][] = array( 'url' => ZAR_URL . "/addons/" . $mod->getVar( 'dirname' ) . "/$path", 'title' => $mod->getVar( 'name' ), 'group' => 'addon' );
    }
}
?>