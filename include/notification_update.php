<?php
// $Id: notification_update.php,v 1.1 2007/03/16 02:39:06 catzwolf Exp $
// ------------------------------------------------------------------------ //
...
// -------------------------------------------------------------------------//
// RMV-NOTIFY
// This addon expects the following arguments:
...
// etc...
// TODO: can we put arguments in the not_redirect argument??? do we need to specially encode them first???
...
// read by this script.  That way a addon can really customize its
...
$user_id = !empty( $zariliaUser ) ? $zariliaUser->getVar( 'uid' ) : 0;
...
foreach ( $update_list as $update_item ) {
    ...
    if ( !$status ) {
        $notification_handler->unsubscribe( $category, $item_id, $event, $addon_id, $user_id );
    } else {
