...
if (!isset($_GET['janrain_nonce']) && ($uname == '' || $pass == '')) {
...
if (isset($_GET['janrain_nonce'])) {
	$user = $xoopsAuth->authenticateWithOpenid();
	$xoopsAuth->getHtmlErrors();
	// was this a new user created and is the activation type 0 or 2
	$auth_errors = $xoopsAuth->getErrors();
	if (array_key_exists(112, $auth_errors) || array_key_exists(113, $auth_errors)) {
		redirect_header(XOOPS_URL.'/user.php', 7, $xoopsAuth->getHtmlErrors());
		exit;
	}
} else {
