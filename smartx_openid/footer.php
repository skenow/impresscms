...
// Hack by marcan to create smarty tags containing information about the curret url
$xoops_urls = smart_getCurrentUrls();
$xoopsTpl->assign('xoops_urls', $xoops_urls);
// End of Hack by marcan to create smarty tags containing information about the curret url

$smarthook_handler->executeHook('footer_start');

/**
 * Hack by marcan <INBOX> for Planetair
 * Including SmartContent MainMenu
 */
$smartcontent_mainmenu_include_file = XOOPS_ROOT_PATH . '/modules/smartcontent/include/mainmenu.inc.php';
if (file_exists($smartcontent_mainmenu_include_file)) {
	include_once($smartcontent_mainmenu_include_file);
}
/**
 * Hack by marcan <INBOX> for Planetair
 * Including SmartContent MainMenu
 ...
 * Multilanguage Hack
 */
//$xoopsTpl->assign('xoops_pagetitle', htmlspecialchars($xoopsConfig['sitename'], ENT_QUOTES));
if ( !defined("XOOPS_FOOTER_INCLUDED") ) {
	...
	if ($xoopsOption['theme_use_smarty'] == 0) {
		// the old way
		$footer = htmlspecialchars( $xoopsConfigMetaFooter['footer'] ) . '<br /><div style="text-align:center">Powered by XOOPS &copy; 2002 <a href="http://www.xoops.org/" target="_blank">The XOOPS Project</a></div>';
