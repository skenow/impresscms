...
class XoopsForm {
	...
	function addElement(&$formElement, $required=false){
		 ...
		 * Add an * when the element is required
		 */
		if($required){
			($formElement->setCaption($formElement->getCaption().'*'));
		}
		/**
	...
	function assign(&$tpl){
		...
		foreach ( $this->getElements() as $ele ) {
		    if(is_object($ele)){
				...
				/*
				 * HAck by felix<inbox> for planetair
				 */
				 if($ele->_extra == " readonly=true"){
					$elements[$n]['body'] = $ele->_value;
				}
				/*
				 * end HAck by felix<inbox> for planetair
				 */
