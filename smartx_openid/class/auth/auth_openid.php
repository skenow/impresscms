...
class XoopsAuthOpenid extends XoopsAuth {
	 ...
	 * @return bool
	 */
    function authenticate($openid_url) {
		$member_handler =& xoops_gethandler('member');
      	$criteria = new CriteriaCompo();
      	$criteria->add(new Criteria('openid_url', $openid_url));
		...
		if ($users && count($users) > 0) {
			if (count($users) > 1) {
				 ...
				 */
				$this->setErrors(101, 'More then one users found.');
				return false;
			}
			return $users[0];
		} else {
			$this->setErrors(100, 'No related openid found');
			return false;
		}
	}
}
