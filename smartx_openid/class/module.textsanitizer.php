...
class MyTextSanitizer
{
	...
	*/
	...
	{
  		global $xoopsConfig, $xoops_language_tags, $xoops_default_language;
		if (!$xoops_language_tags) {
			// The SmartLang module is not installed or no language tags are defined. We return the original text
			return $text;
		}
  		$ret = $this->stripOtherLanguage($text, $xoopsConfig['language']);
  		if (!$ret) {
  			$ret = 	$this->stripOtherLanguage($text, $xoops_default_language);
  		}
		return $ret;
	}

	/**
	 * This is the function that actually strip the text other than selected language
	 *
	 * @param   string  $text
	 * @return  string
	 **/
	function stripOtherLanguage(&$text, $language)
	{
		global $xoops_language_tags;
		static $language_definitions;

		$patterns = array();
		$replacements = array();

		foreach($xoops_language_tags as $tagObj) {
			$patterns[] = "/\[" . $tagObj->tag() ."](.*)\[\/" . $tagObj->tag() ."\]/sU";
  			if ($language == $tagObj->languageid()) {
		  		$replacements[] = '\\1';
  			} else {
	  			$replacements[] = "";
  			}
	...
	// End of ML Hack by marcan
	{
		...
		// End of Hack by marcan for Ampersand Design

		// Hack by marcan for [url] to not open a new window and [urlblank] will be
		$patterns[] = "/\[url=(['\"]?)(http[s]?:\/\/[^\"'<>]*)\\1](.*)\[\/url\]/sU";
		$replacements[] = '<a href="\\2">\\3</a>';
		$patterns[] = "/\[url=(['\"]?)(ftp?:\/\/[^\"'<>]*)\\1](.*)\[\/url\]/sU";
		$replacements[] = '<a href="\\2">\\3</a>';
		$patterns[] = "/\[urlblank=(['\"]?)(http[s]?:\/\/[^\"'<>]*)\\1](.*)\[\/urlblank\]/sU";
		$replacements[] = '<a href="\\2">\\3</a>';
		$patterns[] = "/\[urlblank=(['\"]?)(ftp?:\/\/[^\"'<>]*)\\1](.*)\[\/urlblank\]/sU";
		$replacements[] = '<a href="\\2">\\3</a>';
		// End of Hack by marcan for [url] to not open a new window and [urlblank] will be
		...
		// [siteimg] tag by marcan //
		// hack for images inside the xoops site. Easier site move //
		$patterns[] = "/\[siteimg]([^\"\(\)\?\&'<>]*)\[\/siteimg\]/sU";
		if ($allowimage != 1) {
			$replacements[] = '<a href="'.XOOPS_URL.'/images/\\1" target="_blank">\\1</a>';
		} else {
			$replacements[] = "<div style='text-align: center; padding-top: 5px;'><img src='".XOOPS_URL."/images/\\1' alt='' /></div>";
		}
		// end of [siteimg] tag by marcan //
