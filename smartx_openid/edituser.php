...
if ($op == 'saveuser') {
	...
	if (count($errors) > 0) {
		...
		$last_char = substr($openid_url, strlen($openid_url) - 1, 1);
		if ($last_char == '/') {
			$openid_url = substr($openid_url, 0, strlen($openid_url) - 1);
		}

		$edituser->setVar('openid_url', $openid_url);
...
if ($op == 'editprofile') {
	...
	// echo "<br /><span style='color: #456; margin-bottom: 8px; font-size: 18px; line-height: 18px; font-weight: bold; display: block;'>"._US_EDITPROFILE."</span>";
	echo "<h2>" . _US_EDITPROFILE . "</h2>";
	echo "" . _US_EDITPROFILEINTRO . "";
	...
	$openid_url_text = new XoopsFormText(_US_OPENID_URL, 'openid_url', 30, 2000, $xoopsUser->getVar('openid_url', 'E'));
	$form->addElement($openid_url_text);
