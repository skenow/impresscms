...
if ($action == 'search') {
  ...
  // Added by marcan
	$thetitle = sprintf(_SR_INTRO_TITLE, $xoopsConfig['sitename']);
  echo "<br />";
	echo "<h2>".$thetitle."</h2>";
  echo "". _SR_INTRO_TEXT ."";
    $search_form->display();
	...
	foreach ($mids as $mid) {
        ...
        if ( in_array($mid, $available_modules) ) {
            ...
            /* Hack by marcan (InBox Solutions) for Ampersand Design
 			 * Only display module search result when there actually is resul in that module
 			 */
	        ...
	        // End of Hack by marcan (InBox Solutions) for Ampersand Design
	            for ($i = 0; $i < $count; $i++) {
	                /* Hack by marcan (InBox Solutions) for Ampersand Design
 			 		 * Align the icon verticaly in the middle
 			 		 */
	            	if (isset($results[$i]['image']) && $results[$i]['image'] != "") {
                        //echo "<img src='modules/".$module->getVar('dirname')."/".$results[$i]['image']."' alt='".$myts->makeTboxData4Show($module->getVar('name'))."' />&nbsp;";
                        echo "<img style='padding-bottom: 2px;  padding-top: 2px; vertical-align: middle' src='modules/".$module->getVar('dirname')."/".$results[$i]['image']."' alt='".$module->getVar('name')."' />&nbsp;";
	                } else {
		             ...
		             * Allow HTML in result links
		             */
		            //echo "<b><a href='".$results[$i]['link']."'>".$myts->makeTboxData4Show($results[$i]['title'])."</a></b><br />\n";
		            echo "<b><a href='".$results[$i]['link']."'>".$myts->displayTarea($results[$i]['title'], true)."</a></b><br />\n";
		            /**
...
case 'showallbyuser':
	...
	//	echo "<br />";
		...
		for ($i = 0; $i < $count; $i++) {
            ...
            /**
             * Hack by marcan <INBOX Solutions> for SmartX
             * Do not show any icon if $results[$i]['image'] is explicitly false
             */
			} elseif ($results[$i]['image'] !== false) {
			//} else {
            /**
             * End of Hack by marcan <INBOX Solutions> for SmartX
             * Do not show any icon if $results[$i]['image'] is explicitly false
