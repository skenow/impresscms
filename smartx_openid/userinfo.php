...
if (is_object($xoopsUser)) {
	if ($uid == $xoopsUser->getVar('uid')) {
		...
		// Added for InBox
		$xoopsTpl->assign('lang_notifications', _US_NOTIFICATIONS);
	...
	/**
	 * Hack by marcan <InBox Solutions>¨for CondoNet
	 * Deny access to anonymous user
	 */
	redirect_header("index.php",3,_NOPERM);
	exit();
	/**
	 * End of Hack by marcan <InBox Solutions>¨for CondoNet
	 * Deny access to anonymous user
	 */
...
// Including SmartHelp
/*$smarthelp_include_file = XOOPS_ROOT_PATH . "/modules/smarthelp/include/userinfo.php";
if (file_exists($smarthelp_include_file)) {
	include_once $smarthelp_include_file;
}*/
// Including SmartHelp
 ...
 * Make sure that the Avatar row is not displayed if no avatar has been selected by user
 */
if ($thisUser->getVar('user_avatar') && ($thisUser->getVar('user_avatar') != 'blank.gif')) {
 ...
 * Not display the Web Site if user has none
 */
...
$xoopsTpl->assign('user_signature', $myts->displayTarea($var, 1));
/**
 * Hack by marcan <INBOX Solutions> for SmartX
 * Allow HTML in signature
 */
...
if ($thisUser->getVar('user_viewemail') == 1) {
	...
	/**
	 * Hack by marcan <INBOX Solutions>
	 * vCard in the user profile
	 */
	$xoopsTpl->assign('lang_get_vcard', _US_GET_VCARD);
	/**
	 * End of Hack by marcan <INBOX Solutions>
	 * vCard in the user profile
	 */
