        ...
        if (in_array($i, $read_allowed)) {
			...
			if ((count($sublinks) > 0) && (!empty($xoopsModule)) && ($i == $xoopsModule->getVar('mid'))) {
				foreach($sublinks as $sublink){
					...
					$block['modules'][$i]['sublinks'][] = array('name' => $myts->displayTarea($sublink['name']), 'url' => XOOPS_URL.'/modules/'.$modules[$i]->getVar('dirname').'/'.$sublink['url']);
...
function b_system_themes_show($options)
{
    ...
    foreach ($xoopsConfig['theme_set_allowed'] as $theme) {
        ...
        $theme_options .= '>'.xoops_substr($theme, 0, 20).'</option>';
    }
    $block = array();
    if ($options[0] == 1) {
