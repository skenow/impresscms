        ...
        if ($member_handler->getUserCount(new Criteria('uname', $username)) > 0) {
            ...
            if (!$member_handler->insertUser($newuser)) {
                $adduser_errormsg = _AM_CNRNU;
        	 	...
        	 	/**
			     * Hack by Félix <InBox Solutions> for for Smartx
			     * Correcting xoops bug
			     * set groups properly at user creation on admin side
			     */
            	$error=false;
            	$in_user_grp=false;
            	//insert in each group
				foreach($groups as $group){
					$in_user_grp =  ($group == XOOPS_GROUP_USERS)? true : $in_user_grp;
					if (!$member_handler->addUserToGroup($group, $newuser->getVar('uid'))) {
						$error = true;
					}
				}
				//if not in users group, force it
				if(!$in_user_grp){
                ...
                /**
			     * end of Hack by Félix <InBox Solutions> for Smartx
			     * Correcting xoops bug
			     * set groups properly at user creation on admin side
			     */
            }
