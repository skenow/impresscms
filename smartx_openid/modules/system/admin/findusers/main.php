...
if ( $op == "form" ) {
	...
	/*
     *  Hack by felix<inbox> for smartx
     * 	find users by groups
     */
    $group_select = new XoopsFormSelectGroup(_AM_GROUPS, "group", null, false, 5, true);
	/*
     *  end of Hack by felix<inbox> for smartx
     * 	find users by groups
     */
...
elseif ($op == "submit" & $GLOBALS['xoopsSecurity']->check()) {
	...
	if ( isset($_POST['group']) ) {
		$usersArray = array();
		foreach($_POST['group'] as $grpid){
			$tmpUsersArray = $member_handler->getUsersByGroup($grpid);
			foreach($tmpUsersArray as $userid){
		...
		$uid_string = implode($usersArray, ', ');
		$criteria->add(new Criteria('uid','('.$uid_string.')' , 'IN'));
	}
	/*
     *  end of Hack by felix<inbox> for smartx
     * 	find users by groups
    ...
    if ( $total == 0 ) {
        ...
        if ( $totalpages > 1 ) {
            ...
            foreach($_POST['group'] as $group){
 				$hiddenform .= "<input type='hidden' name='group[]' value='".$group."' />\n";
            }
            /*
		     *  End of Hack by felix<inbox> for smartx
		     * 	find users by groups
